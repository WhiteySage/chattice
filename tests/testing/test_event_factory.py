...
def test_user_and_space_resource_name_coercion() -> None:
    # A str is coerced into a ref carrying that resource name.
    event = EventFactory.message("hi", user="users/custom", space="spaces/custom")
    assert event.actor is not None
    assert event.actor.name == "users/custom"
    assert event.space is not None
    assert event.space.name == "spaces/custom"


def test_user_and_space_refs_pass_through() -> None:
    user = UserRef(name="users/kept", display_name="Kept")
    space = SpaceRef(name="spaces/kept", display_name="Kept Space")
    event = EventFactory.action("deploy.confirm", user=user, space=space)
    assert event.actor is user
    assert event.space is space
