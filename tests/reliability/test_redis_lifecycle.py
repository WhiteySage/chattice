"""Redis lease renewal, client ownership, and JSON value boundaries."""
...
async def test_renew_preserves_the_lease_ttl() -> None:
    """After renew the key must still EXPIRE (a plain
...
async def test_redis_storage_update_merges_data() -> None:
    """The mapping-based storage merges successive updates."""
    redis = fakeredis_aioredis.FakeRedis(decode_responses=True)
