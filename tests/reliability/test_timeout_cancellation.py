...
async def test_timeout_passed_to_sdk() -> None:
    ...
    await _bot(transport).app.messages.create("spaces/AAA", text="x", timeout=7.5)
...
async def test_default_timeout_is_none() -> None:
    ...
    transport = FakeChatTransport(credentials=_creds())
    await _bot(transport).app.messages.create("spaces/AAA", text="x")
    # A default RequestConfig omits timeout entirely — the raw GAPIC
    # default applies (no invented timeout value).
    assert transport.timeouts == [_MethodDefault._DEFAULT_VALUE]
...
async def test_cancellation_propagates() -> None:
    ...
    async def cancel_soon() -> None:
        task = asyncio.ensure_future(
            _bot(transport).app.messages.create("spaces/AAA", text="x")
        )
