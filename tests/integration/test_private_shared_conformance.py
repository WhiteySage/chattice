...
async def test_shared_thread_card_and_state_are_shared_across_two_users() -> None:
    ...
    await _deliver(
        runner,
        _fixture("shared_thread_card_click_user_a.json"),
        "shared-user-a",
    )
    await _deliver(
        runner,
        _fixture("shared_thread_card_click_user_b.json"),
        "shared-user-b",
    )
    ...
    user_a_key = cast(StorageKey, personal_keys[0])
    user_b_key = cast(StorageKey, personal_keys[1])
    assert user_a_key != user_b_key
    assert await storage.get_data(user_a_key) == {"clicks": 1}
    assert await storage.get_data(user_b_key) == {"clicks": 1}
    shared_key = StorageKey(
        user=None,
        space="spaces/TEAM",
