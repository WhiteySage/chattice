...
def test_missing_optional_fields_fall_back_to_defaults() -> None:
    form = ContactForm.from_form_inputs(_inputs(name=StringInput(values=("User A",))))
...
async def test_filter_injects_decoded_form() -> None:
    ...
    outcome = await dispatcher.feed_update(_submit("User A"))
    assert outcome == "User A"
    assert seen and seen[0].name.values == ("User A",)
