...
def _dialog_payload(dialog_type: str) -> dict[str, object]:
    return {
        "type": "CARD_CLICKED",
        "eventTime": "2000-01-01T12:35:00Z",
        "user": {"name": "users/123"},
        ...
        "common": {
            ...
            "formInputs": {"name": {"stringInputs": {"value": ["User A"]}}},
            "userLocale": "en-US",
...
async def test_dialog_response_on_plain_click_returns_500() -> None:
    """Dialog outside a dialog interaction is an application bug."""
    router = Router()
...
async def test_action_status_on_request_dialog_returns_500() -> None:
    """ActionStatus is only valid for SUBMIT_DIALOG."""
    router = Router()
