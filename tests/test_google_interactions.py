...
def test_message_normalization_and_deep_raw_snapshot() -> None:
    ...
    assert event.event_time == datetime(2000, 1, 1, 12, 34, 56, 123456, tzinfo=UTC)
...
def test_form_inputs_preserve_every_documented_variant() -> None:
    ...
    assert isinstance(name, StringInput) and name.values == ("User A",)
...
@pytest.mark.parametrize(
    ...
    [(" deploy prod ", "deploy prod"), ("", ""), (None, "/deploy raw")],
)
def test_slash_command_argument_text_normalization(
    argument_text: str | None, expected: str
) -> None:
    ...
    assert isinstance(event, CommandEvent)
    assert event.message_text == expected
...
def test_lifecycle_widget_and_wrapped_app_home_events() -> None:
    ...
    assert widget.parameters["autocomplete_widget_query"] == "User A"
...
def test_wrapped_submit_form_uses_outer_common_event_object() -> None:
    ...
    assert display_name.values == ("User A",)
        ...
        parse_interaction(
            {
                ...
                "action": {"actionMethodName": "fallback"},
            }
        )
