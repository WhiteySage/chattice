...
async def test_get_message_round_trip() -> None:
    ...
    sent = await _bot(transport).app.messages.create("spaces/AAA", text="stored")
    fetched = await _bot(transport).app.messages.get(sent.name)
...
async def test_get_missing_message_raises_not_found() -> None:
    try:
        await _bot(FakeChatTransport()).app.messages.get("spaces/AAA/messages/nope")
...
async def test_update_message_changes_text() -> None:
    ...
    updated = await _bot(transport).app.messages.update(sent.name, text="after")
...
async def test_update_missing_message_raises_not_found() -> None:
    try:
        await _bot(FakeChatTransport()).app.messages.update(
...
async def test_update_message_card_uses_proto_field_mask() -> None:
    """The gRPC update mask uses the proto
    ...
    transport = FakeChatTransport()
    sent = await _bot(transport).app.messages.create(
        "spaces/AAA", card=Card(header=CardHeader(title="v1"))
    )
    await _bot(transport).app.messages.update(
...
async def test_delete_message_removes_it() -> None:
    transport = FakeChatTransport()
    sent = await _bot(transport).app.messages.create("spaces/AAA", text="doomed")
    await _bot(transport).app.messages.delete(sent.name)
    try:
        await _bot(transport).app.messages.get(sent.name)
    except ChatNotFoundError:
        pass
    else:  # pragma: no cover
...
async def test_get_space_returns_stored_space() -> None:
    ...
    space = await _bot(transport).app.spaces.get("spaces/AAA")
...
async def test_get_missing_space_raises_not_found() -> None:
    try:
        await _bot(FakeChatTransport()).app.spaces.get("spaces/nope")
    except ChatNotFoundError:
        pass
    else:  # pragma: no cover
        raise AssertionError("expected ChatNotFoundError")
