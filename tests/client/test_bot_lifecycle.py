"""Single-flight initialization and linearizable close.
...
async def test_concurrent_first_sends_resolve_provider_once() -> None:
    """Two concurrent sends resolve the provider exactly once."""
    provider = _BlockingProvider()
    transport = FakeChatTransport(credentials=_creds())
    bot = Bot(credentials_provider=provider, transport=transport)

    first = asyncio.create_task(bot.app.messages.create("spaces/AAA", text="a"))
    second = asyncio.create_task(bot.app.messages.create("spaces/AAA", text="b"))
...
async def test_close_during_construction_publishes_no_client() -> None:
    ...
    send = asyncio.create_task(bot.app.messages.create("spaces/AAA", text="x"))
    await asyncio.to_thread(provider.entered.wait, 5)
...
async def test_provider_failure_is_retryable() -> None:
    ...
    await bot.app.messages.create("spaces/AAA", text="x")  # retry succeeds
    assert calls == 2
...
async def test_cancelled_waiter_does_not_kill_shared_init() -> None:
    ...
    cancelled = asyncio.create_task(bot.app.messages.create("spaces/AAA", text="c"))
    await asyncio.to_thread(provider.entered.wait, 5)
    cancelled.cancel()
    ...
    await bot.app.messages.create("spaces/AAA", text="ok")
    assert provider.calls == 1
