"""Accessory widget facade, Bot integration, and action routing."""
...
def test_accessory_widget_to_dict_shape() -> None:
    ...
    # loadIndicator is written explicitly (NONE when unset), so the
...
async def test_bot_send_message_attaches_accessory_widgets() -> None:
    ...
    await bot.app.messages.create(
        "spaces/AAA", text="Invoice", accessory_widgets=[_widget()]
    )
...
async def test_user_auth_rejects_accessory_widgets() -> None:
    ...
    with pytest.raises(CapabilityNotSupported, match="app authentication"):
        await bot.app.messages.create(
            "spaces/AAA", text="x", accessory_widgets=[_widget()]
        )
