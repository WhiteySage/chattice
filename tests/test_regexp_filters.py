...
async def test_regexp_match_from_start() -> None:
    filt = F.text.regexp(r"^[Cc]afé$")
    assert await filt(_event("Café"), {}) is True
    assert await filt(_event("café"), {}) is True
    assert await filt(_event("café2"), {}) is False
    assert await filt(_event(" my café"), {}) is False


async def test_regexp_flags() -> None:
    filt = F.text.regexp(r"^café$", flags=re.IGNORECASE)
    assert await filt(_event("CAFÉ"), {}) is True
...
async def test_regexp_equality_stays_literal() -> None:
    ...
    assert await (F.text == r"^café$")(event, {}) is False
