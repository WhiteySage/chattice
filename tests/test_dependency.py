...
async def test_missing_required_dependency_has_clear_error() -> None:
    ...
    with pytest.raises(DependencyResolutionError, match="database") as caught:
        await dispatcher.feed_update(MessageEvent())
    message = str(caught.value)
    assert (
        "test_missing_required_dependency_has_clear_error.<locals>.handler" in message
    )
    assert "MessageEvent" in message
...
def test_non_async_handler_is_rejected_at_registration() -> None:
    router = Router()
