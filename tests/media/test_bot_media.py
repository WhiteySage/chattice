...
async def test_upload_attachment_user_auth(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    uploaded = await _bot(_UserCreds()).user.attachments.upload(
...
async def test_upload_attachment_app_auth_rejected() -> None:
    with pytest.raises(CapabilityNotSupported, match="USER identity is not configured"):
        await _bot(_AppCreds()).user.attachments.upload(
...
async def test_upload_attachment_dual_identity_uses_user_provider(
    ...
    await bot.user.attachments.upload(
        "S1", InputFile.from_bytes(b"x", filename="x.png")
    )
...
async def test_upload_attachment_provider_returned_service_account_rejected() -> None:
    ...
    with pytest.raises(CapabilityNotSupported, match="Requested USER identity"):
        await bot.user.attachments.upload(
            "S1", InputFile.from_bytes(b"x", filename="x.png")
        )
...
async def test_upload_attachment_delegated_dwd_credentials_work(
    ...
    uploaded = await bot.user.attachments.upload(
        "S1", InputFile.from_bytes(b"x", filename="x.png")
    )
...
async def test_upload_attachment_user_without_scope_rejected() -> None:
    ...
    with pytest.raises(CapabilityNotSupported, match="MEDIA_UPLOAD"):
        await _bot(NarrowUserCreds()).user.attachments.upload(
            "S1", InputFile.from_bytes(b"x", filename="x.png")
        )
...
async def test_upload_attachment_missing_media_extra(
    ...
    with pytest.raises(ChatAPIError, match=r"chattice\[media\]"):
        await _bot(_UserCreds()).user.attachments.upload(
            "S1", InputFile.from_bytes(b"x", filename="x.png")
        )
...
async def test_download_attachment_bytes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    data = await _bot(_AppCreds()).app.attachments.download("media/r1")
...
async def test_download_attachment_destination(
    ...
    result = await _bot(_UserCreds()).user.attachments.download(
...
async def test_download_identity_namespaces_are_explicit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Download accepts either namespace; the caller picks the identity."""
    captured: list[Credentials] = []
    ...
    with pytest.raises(CapabilityNotSupported, match="MEDIA_DOWNLOAD"):
        await bot.app.attachments.download("media/r1")
    assert captured == []  # rejected locally, no REST call
    data = await bot.user.attachments.download("media/r1")
    assert data == b"content"
    assert captured[0] is user_credentials


async def test_upload_without_user_identity_rejected() -> None:
...
async def test_download_drive_backed_rejected() -> None:
    ...
    with pytest.raises(ChatAPIError, match="Drive"):
        await _bot(_AppCreds()).app.attachments.download(ref)
...
async def test_get_attachment_app_auth() -> None:
    ...
    ref = await bot.app.attachments.get_metadata(name)
...
async def test_get_attachment_unknown_name_wraps_not_found() -> None:
    ...
    with pytest.raises(ChatNotFoundError):
        await bot.app.attachments.get_metadata(
            "spaces/S/messages/M/attachments/missing"
        )


async def test_get_attachment_user_auth_rejected() -> None:
    with pytest.raises(CapabilityNotSupported, match="Requested APP identity"):
        await _bot(_UserCreds()).app.attachments.get_metadata(
...
async def test_send_message_rejects_cross_space_uploaded() -> None:
    ...
    with pytest.raises(ChatAPIError, match="scoped"):
        await bot.user.messages.create("spaces/A", attachments=[uploaded])
    assert transport.calls == []
