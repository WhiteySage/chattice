"""Frozen facades snapshot mutable inputs and validate
...
async def test_raw_widget_nested_mutation_does_not_change_widget() -> None:
    """Mutating the original nested dict after
...
def test_dialog_rejects_datetime_picker_through_raw_widget() -> None:
    """The dialog rule is enforced on the serialized tree: a
...
def test_workspace_event_deep_snapshots_data_and_raw() -> None:
    """Mutating the caller's nested values after parsing must not
