...
async def test_provider_called_exactly_once(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    await bot.app.messages.create("spaces/AAA", text="y")
...
async def test_provider_wins_over_credentials(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    await bot.app.messages.create("spaces/AAA", text="x")
    assert provider.calls == 1
    client = cast(_FakeAsyncClient, await bot.raw.app())
...
async def test_implicit_mode_provider_called_once_across_uses(
    ...
    assert bot.auth_mode is AuthMode.APP  # triggers classification (SA -> APP)
    assert bot.auth_mode is AuthMode.APP  # second access must stay cached
    assert provider.calls == 1
