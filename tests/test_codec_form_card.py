"""Enum codec, App Home form reuse, and card round trips."""
...
def test_numeric_enum_roundtrip() -> None:
    """Numeric Enum values used to raise
...
def test_form_filter_accepts_app_home_form_submit() -> None:
    ...
    async def run() -> dict[str, object]:
        event = FormSubmitEvent(
            ...
            form_inputs=FormInputs(data={"name": StringInput(values=("User A",))}),
        )
    ...
    assert result["form"].name.values == ("User A",)
