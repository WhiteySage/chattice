...
The documentation verification also copies this file outside the checkout and runs it
...
def message_payload(text: str) -> dict[str, object]:
    ...
    return {
        "type": "MESSAGE",
        "user": {"name": "users/user-a"},
        "space": {"name": "spaces/AAA"},
        "message": {
            "name": "spaces/AAA/messages/1",
            "text": text,
            "sender": {"name": "users/user-a", "type": "HUMAN"},
        ...
        parse_interaction(
            {
                ...
                "common": {
                    ...
                    "formInputs": {
                        "email": {"stringInputs": {"value": ["email-value"]}}
                    },
                },
            }
        )
    )
    assert form_result == ActionStatus.ok("Saved email-value")
