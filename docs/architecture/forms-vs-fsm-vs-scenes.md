...
Status: **Established design guidance**.
...
async def submit(event: ActionEvent, form: RegistrationForm) -> ActionStatus:
    return ActionStatus.ok(f"Saved: {form.name.values[0]}")
```
...
message-by-message registration is an incremental alternative, not the
Google-native first choice (see the FSM workflow in
`examples/finite_state_machine.py` vs the typed Dialog alternative in
`examples/docs/from_zero.py`).

## Scenes — organize longer workflows

Scenes can layer over FSM to structure long multi-step workflows
(enter/exit/fallback/timeout/reset). A Scene is never a second persistence
engine.
...
Canonical examples: `examples/docs/from_zero.py` (typed form + dialog)
and `examples/finite_state_machine.py` (sequential FSM). Polls are
application-owned scenarios assembled from the primitives.
...
The interaction response (30 s, no outbound credentials) and authenticated `Bot` calls
...
async def welcome(event: AddedToSpaceEvent) -> str:
    return "Hi! I can: /search — search, Report — export, buttons — requests."
```
...
- `return "pong"` / `InteractionResponse.respond(...)` — answer the
  CURRENT interaction (30 s, no outbound credentials). `bot.app.messages.create(space, ...)` —
