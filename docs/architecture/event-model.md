...
Framework events are lightweight immutable Python values. Focused optional
interaction metadata lives on a keyword-only base:
...
- `ErrorEvent(source_event=..., exception=...)` — framework error-routing
...
The adapter preserves a future external type without making it fatal:
...
Pydantic validates untrusted data only inside the interaction adapter. Known
