...
| `chattice.routing` | Event receipt, observer selection, handler matches, and result types at DEBUG |
...
| `chattice.pubsub` | Pull delivery lifecycle, answer mode/success, and failure diagnostics and DEBUG tracebacks |
...
- Events expose their original payload through `.raw`; the context, handler
  result, and exception can also contain application data. Export only the
  fields needed for diagnostics. The example records exception types without
  their messages.
