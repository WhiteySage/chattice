...
Status: **Implemented**. Google Pub/Sub delivers Chat interactions
...
react use the Chat API asynchronously: `MESSAGES_CREATE` / `MESSAGES_UPDATE`
...
- handler answers go outbound via `Bot` (`str` → send, `Card` →
  ...
  `CapabilityNotSupported` and the space is told;
