...
against the official event guides): message
...
`Dispatcher.feed_update()` rejects `WorkspaceEvent`; applications use the
separate runtime and call `feed_event()` instead. Handlers must be defined at
...
Applications provide their own client for subscription creation and renewal;
the framework does not include a lifecycle manager.
