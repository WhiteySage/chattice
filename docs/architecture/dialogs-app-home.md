...
Chattice supports two interaction-driven UI surfaces that go beyond plain
...
- **Dialogs are interaction-only.** A `Dialog` response is valid for a command
  or a `REQUEST_DIALOG` action. `SUBMIT_DIALOG` accepts `ActionStatus`;
  `CANCEL_DIALOG` normally returns `None`. Submit/cancel events cannot open a
  new Dialog. Invalid typed responses are rejected with a 500.
