...
| `message.answer()` | handler return, `MessageEvent.reply()`, `ThreadRef.send()`, `SpaceRef.send()`, or `bot.app.messages.create()` | A sync HTTP response and an authenticated Chat API call are different channels |
