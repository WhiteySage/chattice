...
### Common observer filters

Apply one scope to every handler registered on an observer:

```python
reports = Router(name="reports")

reports.message.filter(F.space.name.in_({"spaces/AAA", "spaces/BBB"}))


@reports.message(F.text == "report")
async def report(message: MessageEvent) -> str:
    return "report"


@reports.message(F.text == "status")
async def status(message: MessageEvent) -> str:
    return "ready"
```

Repeated `observer.filter(...)` calls append filters and combine them as AND.
Common filters run once before handler-specific filters. A mapping returned by
a common filter becomes dependency context for handler filters and the
handler itself; the existing context-conflict rules still apply.

Observer filters are local to that observer and Router. They do not leak to
another observer or inherit into child Routers. If no specific message
handler matches, the existing generic `event` fallback remains available.

Direct-message scoping uses the same magic-filter API:

```python
assistant.message.filter(F.space.space_type == "DIRECT_MESSAGE")
```
...
@support.message(F.text.regexp(r"^[Tt]est$"))
async def test_command(message: MessageEvent) -> str:
    return "ok"


@support.message(F.text.regexp(r"^test$", flags=re.IGNORECASE))
...
match. String equality stays literal: `F.text == "^test$"` compares the
...
FSM records scope per-user workflow state (USER_IN_SPACE / USER /
