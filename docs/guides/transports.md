...
| Handler return value | serialized to Chat | ignored (push) / mapped outbound where supported (pull runner) |
...
Set `CHATTICE_SERVICE_ACCOUNT_FILE` and `CHATTICE_SUBSCRIPTION`, then run
`python app.py`. The service account needs the Pub/Sub Subscriber IAM role;
the example supplies subscriber and Chat credentials separately. The method name is deliberately explicit:
...
- **One topic, many consumers** — attach extra subscriptions (compliance,
