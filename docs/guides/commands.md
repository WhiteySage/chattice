...
async def deploy(event: CommandEvent) -> str:
    ...
    environment = event.message_text or ""
...
`CommandEvent` and strips surrounding whitespace from `argumentText`. It does
not parse mentions or `/deploy` out of ordinary message text.
