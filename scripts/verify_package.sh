...
# The expected version comes from pyproject.toml (single source of truth)
# instead of being duplicated here, so a version bump cannot silently
# desynchronize the check below.
EXPECTED_VERSION="$("$TMP_VENV/bin/python" -c 'import tomllib; print(tomllib.load(open("pyproject.toml", "rb"))["project"]["version"])')"
...
assert chattice.__version__ == os.environ["EXPECTED_VERSION"], chattice.__version__
