# ruff: noqa: ASYNC109 — timeout kwarg mirrors the resource client call convention
...
def _message_sender(explicit: object | None) -> Any:
    ...
    if sender is None or not hasattr(sender, "app"):
        raise RuntimeError(
...
async def _send_message(
    ...
    *,
    ...
    # The contextual path shares the resource facade: APP namespace by
    # default (attachment sends are USER-gated there with an actionable
    # hint, never silently rerouted).
    from google.apps.chat_v1.types.message import CreateMessageRequest

    option = (
        reply_option
        if reply_option is not None
        else (
            CreateMessageRequest.MessageReplyOption.REPLY_MESSAGE_FALLBACK_TO_NEW_THREAD
        )
    )
    sender = _message_sender(explicit)
    result = await sender.app.messages.create(
        space,
        text,
        thread=thread,
        reply_option=option,
