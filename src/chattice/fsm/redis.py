...
Redis commands are individually atomic. ``update_data`` performs GET -> SET
under a process-local lock, so it does not prevent cross-process races. Use
``set_data`` for replacement semantics. For cross-process compare-and-set
behavior, use ``BaseStorageFromRecord(RedisFSMRecordStorage(...))``.
"""
